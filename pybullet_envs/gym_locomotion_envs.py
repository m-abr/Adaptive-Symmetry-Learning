...
class WalkerBaseBulletEnv(MJCFBaseBulletEnv):
  ...
  def __init__(self, robot, render=False):
    ...
    self.targets = [target_angles[i] for i in Config.ENV_params["available_targets"]] 
    self.target_counter = 0
    self.inclination = Config.ENV_params["inclination"] * np.pi/180 # convert to radians
    self.tilt_ori = target_angles[Config.ENV_params["tilt_orientation"]]
  ...
  def reset(self):
    if (self.stateId >= 0):
      ...
      po = self._p.getBasePositionAndOrientation(1)[0]
      target = self.targets[self.target_counter % len(self.targets)]
      self.target_counter += 1
      self._p.resetBasePositionAndOrientation(1,po,self._p.getQuaternionFromEuler((0,0,target)))
      self._p.resetDebugVisualizerCamera(5,-40,-30,(3,0,0)) # distance, yaw, pitch, target pos

    r = MJCFBaseBulletEnv.reset(self)
    if (self.stateId >= 0) and self.inclination != 0: # tilted surface
      ori = target - self.tilt_ori # add target to compensate for initial robot rotation
      z = -np.cos(self.inclination)*9.8
      x = np.cos(ori)*np.sin(self.inclination)*9.8
      y = np.sin(ori)*np.sin(self.inclination)*9.8
      self._p.setGravity(x,y,z)
