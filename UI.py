...
class UI():
    ...
    def read_int(prompt, min, max):
        ...
        :param min: mininum input (inclusive)
    ...
    @staticmethod
    def print_table(data, titles=None, alignment=None, cols_width=None, margins=None, numbering=None, prompt=None):
        '''
        Print table
        ...
        -------
        index : `int`
            returns global index of selected item
        col_index : `int`
            returns number of column of selected item (starts at 0)
        column : `int`
            returns column of selected item
        * if `numbering` or `prompt` are `None`, `None` is returned
        ...
        aligment = ["<","^"]               # 1st column is left-aligned, 2nd is centered
        ...
        for i,n in enumerate(cumulative_item_per_col):
            if index < n:
                return index, index-n+len(data[i]), i 
